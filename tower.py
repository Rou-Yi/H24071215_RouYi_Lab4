...
class Circle:
    ...
    def collide(self, enemy):
        ...
        x0, y0 = self.center
        x1, y1 = enemy.get_pos()
        # calculate the distance from enemy to the center
        distance = math.sqrt((x0 - x1) ** 2 + (y0 - y1) ** 2)
        # judge the distance is in range or not
    ...
    def draw_transparent(self, win):
        ...
        pygame.draw.circle(transparent_surface, (0, 0, 0, transparency), self.center, self.radius)
...
class Tower:
    ...
    def is_cool_down(self):
        ...
        if self.cd_count < self.cd_max_count:
            ...
            self.cd_count = 0  # reset cd time to be 0
            return True
    ...
    def attack(self, enemy_group):
        ...
        :return: None
        """
        if self.is_cool_down():  # check cd time is enough
            for en in enemy_group.get():
                if self.range_circle.collide(en):  # check the enemy is in attack range
                    en.get_hurt(self.damage)  # cause damage
                    break  # stop attack if tower has attacked any enemy
